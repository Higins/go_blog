...
func (facultyModel FacultyModel) getAllBlog(c *fiber.Ctx, ([]entities.Faculty, ) {
	db := DBConn
	...
	db.Debug().Table("blogs b").
		Select("b.title,b.text,count(c.id) as commentNumber,b.id ").
		Joins("left join commensts c on b.id = c.blog_id").
		Group("b.id").
		Find(&faculties)
