...
func getAllBlog(c *fiber.Ctx) {
	...
	blog := []Blog{}
	//blogs := db.Find(&blog)
	db.Table("blogs b").Select("b.title,b.text,count(c.id) as commentNumber,b.id ").Joins("left join commensts c on b.id = c.blog_id").Group("b.id").Scan(&blog)
	blog.commentNumber = append(blog.Commenst, CommenstItem)
	c.JSON(blog)
}
