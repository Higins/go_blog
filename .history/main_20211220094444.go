...
func getAllBlog(c *fiber.Ctx) {
	...
	//blog := []Blog{}
	//blogs := db.Find(&blog)
	blogs2 := db.Table("blogs b").Select("b.title,b.text,count(c.id) as commentNumber ").Joins("left join commensts c on b.id = c.blog_id").Group("b.id").Scan(&Blog{})
	for i := 0; i < int(blogs2.RowsAffected; i++ {
		fmt.Println(blogs2[i])
	}
	c.JSON(blogs2)
}
